...
?>
<form action="index.php?module=kesenian&action=proses" method="post" enctype="multipart/form-data">
<?php if(@$_GET['id'] !=''){ ?>
...
<table>
	<?php
		$id = @$_GET['id'];
		$sql=$db->prepare("SELECT * FROM kesenian WHERE id = :id ");
		$sql->bindParam(':id', $id);
		$sql->execute();
...
<button type="button" class="btn btn-danger" onclick="javascript:window.location.href='index.php?module=kesenian'">CANCEL</button>
</form>
<?php
	}else{
